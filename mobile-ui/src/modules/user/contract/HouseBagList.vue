<template>
    ...
    <div id="listConter" v-if="houseList=='submit'">
        ...
        <div class="ver-code-bottom-one-right-code pic-area">
          <div class="posting-uploader-item" v-for="(item,index) in list.houseImg" :key="index">
        ...
        <div class="listConter" v-else-if="houseList=='end'">
...
export default {
    ...
    methods:{
        submitList(){
            if(!this.list.listdec || this.list.houseImg.length==0){
        ...
        let param = {
          ...
          house_items: this.list.listdec,
          house_items_imgs:"123"
        };
        this.$http.post(this.$store.state.global.baseUrl + 'user/check_in', param).then(res => {
          //debugger
          if(res.status == 200) {
        ...
        afterZRead(file){
          this.list.houseImg.push(file.content); 
      },
        onClickLeft() {
            this.$router.back(-1);
        },
        delImg(index){
       console.log(this.list.houseImg.length);
        if(isNaN(index) || index >= this.list.houseImg.length){
          return false;
        }
        let tmp = [];
        for(let i=0,len = this.list.houseImg.length;i<len;i++){
          if (this.list.houseImg[i] !== this.list.houseImg[index]) {
            tmp.push(this.list.houseImg[i])
          }
        }
        this.list.houseImg = tmp
      },
    }
}
</script>
